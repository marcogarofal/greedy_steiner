...
def find_best_solution_overall(graph, weak_nodes, mandatory_nodes, all_discretionary_nodes, power_capacities):
    """
    Find the best solution by testing all possible combinations of discretionary nodes
    """
    print(f"\n{'='*60}")
    print(f"SEARCHING FOR BEST ABSOLUTE SOLUTION")
    print(f"{'='*60}")
    ...
    # 2. Solutions with all possible combinations of discretionary nodes
    print(f"\n--- Testing discretionary node combinations ---")
    
    # Generate all possible combinations (from 1 to all)
    for r in range(1, len(all_discretionary_nodes) + 1):
        for discretionary_subset in combinations(all_discretionary_nodes, r):
            discretionary_list = list(discretionary_subset)
            print(f"\nTesting with discretionary: {discretionary_list}")
            
            solution = solve_with_discretionary_subset(
                graph, weak_nodes, mandatory_nodes, discretionary_list, power_capacities.copy(),
                f"WITH discretionary {discretionary_list}"
            )
            all_solutions.append(solution)
            print(f"  Score: {solution.score:.2f}, Connected: {len(solution.connected_weak)}/{len(weak_nodes)}, Cost: {solution.total_cost}")
    
    # 3. Find best solution
    best_solution = min(all_solutions, key=lambda s: s.score)
    
    print(f"\n{'='*60}")
    print(f"COMPARING ALL SOLUTIONS")
    print(f"{'='*60}")
    
    # Sort and show all solutions
...
def visualize_best_solution(graph, best_solution, weak_nodes, mandatory_nodes, all_discretionary_nodes, save_name="BEST_SOLUTION"):
    ...
    # Handle weight labels (as in previous version)
    ...
    # Detailed title
    title = (f"🏆 BEST ABSOLUTE SOLUTION\n"
...
def save_solution_summary(best_solution, all_solutions, save_name="solution_summary"):
    """
    Save a text summary of all solutions
    """
    with open(f'{path}{save_name}.txt', 'w') as f:
        f.write("="*80 + "\n")
        f.write("COMPLETE SOLUTION SUMMARY\n")
        f.write("="*80 + "\n\n")
        ...
        f.write("-"*40 + "\n")
        f.write("Note: 'Discretionary used' shows only those ACTUALLY used in the solution\n\n")
        
        for i, solution in enumerate(all_solutions):
            f.write(f"#{i+1:2d} - Score: {solution.score:7.2f} | ")
...
if __name__ == "__main__":
    ...
    for graph_idx, graph in enumerate(loaded_graphs):
        print(f"\n{'='*80}")
        print(f"PROCESSING GRAPH {graph_idx + 1}/4")
        print(f"{'='*80}")
        ...
        # FIND BEST SOLUTION AMONG ALL ALTERNATIVES
        best_solution, all_solutions = find_best_solution_overall(
        ...
        visualize_best_solution(graph, best_solution, weak_nodes_list, mandatory_nodes_list, 
                               discretionary_nodes_list, f"GRAPH_{graph_idx+1}_BEST_SOLUTION")
        
        # SAVE SUMMARY
        save_solution_summary(best_solution, all_solutions, f"GRAPH_{graph_idx+1}_summary")
        
        print(f"\n🏆 GRAPH {graph_idx + 1} COMPLETED")
