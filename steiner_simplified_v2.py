...
class ConfigurationLogger:
    ...
    def save_detailed_log(self, filename=None):
        ...
        with open(filepath, 'w') as f:
            ...
            valid_configs = [(i, config) for i, config in enumerate(self.results)
                           if config['solutions'] and min(config['solutions'], key=lambda s: s['score'])['failed_connections'] == 0]

            if valid_configs:
                valid_configs.sort(key=lambda x: min(x[1]['solutions'], key=lambda s: s['score'])['score'])

                f.write("TOP 5 BEST PERFORMING CONFIGURATIONS (by score):\n")
                f.write("-" * 60 + "\n")
                for i, (config_idx, config) in enumerate(valid_configs[:5]):
                    best_sol = min(config['solutions'], key=lambda s: s['score'])
                    f.write(f"#{i+1} Config #{config_idx+1}: Score {best_sol['score']:.2f}, "
                           f"Alpha {config['parameters'].get('alpha', 'N/A')}, "
                           f"Discretionary {best_sol['discretionary_used']}\n")
                f.write("\n")

            # Alpha parameter analysis
            alpha_results = {}
            for config in self.results:
                alpha = config['parameters'].get('alpha')
                if alpha is not None and config['solutions']:
                    best_sol = min(config['solutions'], key=lambda s: s['score'])
                    if alpha not in alpha_results:
                        alpha_results[alpha] = []
                    alpha_results[alpha].append(best_sol['score'])

            if alpha_results:
                f.write("ALPHA PARAMETER IMPACT ANALYSIS:\n")
                f.write("-" * 40 + "\n")
                for alpha in sorted(alpha_results.keys()):
                    ...
                    f.write(f"Alpha {alpha}: Avg={avg_score:.2f}, Min={min_score:.2f}, Max={max_score:.2f} ({len(scores)} tests)\n")
                f.write("\n")
            ...
            for config in self.results:
                if config['solutions']:
                    best_sol = min(config['solutions'], key=lambda s: s['score'])
                    pattern = tuple(sorted(best_sol['discretionary_used']))
                    if pattern not in discretionary_patterns:
                        discretionary_patterns[pattern] = []
                    discretionary_patterns[pattern].append(best_sol['score'])

            if discretionary_patterns:
                f.write("DISCRETIONARY USAGE PATTERNS:\n")
                f.write("-" * 35 + "\n")
                sorted_patterns = sorted(discretionary_patterns.items(),
                                       key=lambda x: sum(x[1])/len(x[1]))

                for pattern, scores in sorted_patterns:
                    avg_score = sum(scores) / len(scores)
                    f.write(f"Pattern {list(pattern)}: Avg Score {avg_score:.2f} ({len(scores)} occurrences)\n")
...
def find_balanced_overload_solution(remaining_weak, all_weak_options, current_capacity_usage,
                                   power_capacities, alpha=0.5):
    """
    Find the best way to connect remaining weak nodes by balancing overload across power nodes.

    IMPROVED VERSION: Also considers using unused discretionary nodes to reduce overload

    Uses a greedy approach that considers:
    1. Current overload levels of power nodes
    2. Cost of paths (both ACC and AOC components)
    3. Minimizing maximum overload across all nodes
    4. ACTIVELY using unused discretionary nodes to prevent overload

    Returns a dictionary: {weak_node: chosen_path_info}
    """
    print(f"    🔍 Finding balanced overload solution for {len(remaining_weak)} nodes...")

    # Check if there are overloaded nodes and unused discretionary capacity
    overloaded_nodes = []
    unused_discretionary_capacity = 0

    for node, usage in current_capacity_usage.items():
        max_cap = power_capacities.get(node, float('inf'))
        if usage > max_cap and max_cap != float('inf'):
            overload = usage - max_cap
            overloaded_nodes.append((node, overload))

    # Find discretionary nodes that are completely unused
    unused_discretionary = []
    for node in power_capacities:
        if node not in current_capacity_usage or current_capacity_usage[node] == 0:
            if power_capacities[node] > 0:  # Has capacity
                unused_discretionary.append(node)
                unused_discretionary_capacity += power_capacities[node]

    if overloaded_nodes and unused_discretionary:
        total_overload = sum(overload for _, overload in overloaded_nodes)
        print(f"    ⚠️  DETECTED INEFFICIENCY:")
        print(f"        - Overloaded nodes: {[(node, f'+{overload}') for node, overload in overloaded_nodes]}")
        print(f"        - Total overload: {total_overload}")
        print(f"        - Unused discretionary nodes: {unused_discretionary}")
        print(f"        - Unused discretionary capacity: {unused_discretionary_capacity}")

        if unused_discretionary_capacity >= total_overload:
            print(f"    💡 OPTIMIZATION OPPORTUNITY: Can eliminate all overload using unused discretionary!")
        else:
            print(f"    💡 OPTIMIZATION OPPORTUNITY: Can reduce overload from {total_overload} to {total_overload - unused_discretionary_capacity}")

    # Create a copy of capacity usage to simulate additions
    sim_capacity_usage = current_capacity_usage.copy()
    connections = {}

    # Sort weak nodes by number of available paths (fewer options = higher priority)
    weak_nodes_sorted = sorted(remaining_weak,
                              key=lambda w: len(all_weak_options.get(w, [])))

    for weak_node in weak_nodes_sorted:
        available_paths = all_weak_options.get(weak_node, [])

        if not available_paths:
            connections[weak_node] = None
            continue

        # Evaluate each path considering current overload distribution
        path_scores = []
        ...
        for path_info in available_paths:
            ...
            # Calculate overload impact on target mandatory node
            current_usage = sim_capacity_usage.get(target_mandatory, 0)
            max_capacity = power_capacities.get(target_mandatory, float('inf'))

            # Current overload of this node
            current_overload = max(0, current_usage - max_capacity) if max_capacity != float('inf') else 0
            # Overload after adding this connection
            new_overload = max(0, (current_usage + 1) - max_capacity) if max_capacity != float('inf') else 0

            # IMPROVED: Strong penalty for adding to already overloaded nodes
            # when unused discretionary capacity is available
            if current_overload > 0 and unused_discretionary_capacity > 0:
                # Check if this path uses any unused discretionary nodes
                uses_unused_discretionary = any(disc in unused_discretionary for disc in discretionary_used)

                if not uses_unused_discretionary:
                    # Heavy penalty for adding to overloaded node when unused discretionary exists
                    overload_penalty = current_overload * 50 + (new_overload - current_overload) * 100
                else:
                    # Light penalty if using unused discretionary (encouraged)
                    overload_penalty = current_overload * 5 + (new_overload - current_overload) * 10
            else:
                # Standard penalty calculation
                overload_penalty = current_overload * 10 + (new_overload - current_overload) * 5

            # Check discretionary nodes overload too
            disc_overload_penalty = 0
            for disc_node in discretionary_used:
                disc_current = sim_capacity_usage.get(disc_node, 0)
                disc_max = power_capacities.get(disc_node, float('inf'))
                if disc_max != float('inf'):
                    disc_overload = max(0, disc_current - disc_max)
                    disc_new_overload = max(0, (disc_current + 1) - disc_max)

                    # Bonus for using completely unused discretionary nodes
                    if disc_current == 0 and disc_node in unused_discretionary:
                        disc_overload_penalty -= 20  # Negative penalty = bonus
                    else:
                        disc_overload_penalty += disc_overload * 5 + (disc_new_overload - disc_overload) * 3

            # Path cost (ACC + AOC components)
            path_edges = [(path_info['path'][i], path_info['path'][i+1])
                         for i in range(len(path_info['path'])-1)]

            edge_weight_sum = sum(main_graph[u][v]['weight'] for u, v in path_edges)

            # AOC increment using CORRECT formula: p_j * d_j
            # p_j = capacity of target mandatory node
            p_target = power_capacities.get(target_mandatory, 0)
            # d_j = degree increment (adding 1 connection to target)
            aoc_increment = p_target * 1  # Each connection adds degree 1

            # Add AOC increment for discretionary nodes used in path
            for disc_node in discretionary_used:
                p_disc = power_capacities.get(disc_node, 0)
                # Discretionary nodes in path get 2 connections (input + output)
                aoc_increment += p_disc * 2

            # Combined path cost
            n = len(main_graph.nodes())
            incremental_acc = edge_weight_sum / (n * (n - 1)) if n > 1 else 0
            incremental_aoc = aoc_increment / n if n > 0 else 0
            path_cost = alpha * incremental_acc + (1 - alpha) * incremental_aoc

            # Total score: balance path cost with overload distribution
            # Higher weight on overload to prioritize balancing
            total_score = path_cost * 100 + overload_penalty + disc_overload_penalty

            path_scores.append({
                'path_info': path_info,
                'score': total_score,
                'overload_penalty': overload_penalty + disc_overload_penalty,
                'path_cost': path_cost,
                'target_node': target_mandatory,
                'new_overload': new_overload,
                'uses_unused_discretionary': any(disc in unused_discretionary for disc in discretionary_used)
            })

        # Choose the path with the best score (lowest = better)
        if path_scores:
            best_path = min(path_scores, key=lambda x: x['score'])
            chosen_path = best_path['path_info']
            connections[weak_node] = chosen_path

            # Update simulated capacity usage
            target = chosen_path['target_mandatory']
            sim_capacity_usage[target] = sim_capacity_usage.get(target, 0) + 1

            for disc_node in chosen_path['discretionary_used']:
                sim_capacity_usage[disc_node] = sim_capacity_usage.get(disc_node, 0) + 1
                # Remove from unused list if now being used
                if disc_node in unused_discretionary:
                    unused_discretionary.remove(disc_node)
                    unused_discretionary_capacity -= power_capacities.get(disc_node, 0)

            optimization_marker = "🎯" if best_path['uses_unused_discretionary'] else "📊"
            print(f"    {optimization_marker} {weak_node} → {chosen_path['path']} "
                  f"(score: {best_path['score']:.2f}, "
                  f"overload_penalty: {best_path['overload_penalty']:.1f}, "
                  f"target_usage: {sim_capacity_usage[target]}/{power_capacities.get(target, '∞')})")
        else:
            connections[weak_node] = None

    # Show final overload distribution with improvement analysis
    print(f"    📈 Final overload distribution:")
    final_overloaded = []
    final_total_overload = 0

    for node in sorted(power_capacities.keys()):
        if node in sim_capacity_usage:
            usage = sim_capacity_usage[node]
            capacity = power_capacities[node]
            overload = max(0, usage - capacity) if capacity != float('inf') else 0
            if overload > 0:
                final_overloaded.append((node, overload))
                final_total_overload += overload
            status = f"⚠️ +{overload}" if overload > 0 else "✅"
            print(f"        Node {node}: {usage}/{capacity} {status}")

    # Show improvement summary
    if overloaded_nodes:
        initial_total_overload = sum(overload for _, overload in overloaded_nodes)
        improvement = initial_total_overload - final_total_overload
        print(f"    📊 OVERLOAD IMPROVEMENT: {initial_total_overload} → {final_total_overload} (reduction: {improvement})")

        if final_total_overload == 0:
            print(f"    🎉 SUCCESS: All overload eliminated using unused discretionary nodes!")
        elif improvement > 0:
            print(f"    ✅ IMPROVEMENT: Reduced overload by {improvement} units")
        else:
            print(f"    ⚠️ NO IMPROVEMENT: Consider manual path adjustments")
...
def run_multiple_configurations(save_individual_files=False, alpha=0.5):
    """
    Run multiple configurations with different parameters

    Args:
        save_individual_files: If True, saves individual PNG/TXT for each config
        alpha: The alpha parameter to use for ALL configurations (user controlled)
    """
    print(f"\n{'='*80}")
    print(f"RUNNING MULTIPLE CONFIGURATION TESTS")
    ...
    # Configuration sets to test - FOCUS ON PRACTICAL VARIATIONS
    configurations = [
        # Baseline configuration
        {
            'name': 'Baseline',
            'graph_index': 0,
            'alpha': alpha,  # Use user-specified alpha
            'capacities': {1: 1, 2: 1, 3: 1, 4: 3, 5: 3, 6: 5, 7: 0},
        },

        # CAPACITY VARIATIONS (more practical)
        {
            'name': 'High_Capacity',
            'graph_index': 0,
            'alpha': alpha,
            'capacities': {1: 2, 2: 2, 3: 2, 4: 5, 5: 5, 6: 8, 7: 1},  # Higher capacities
        },
        {
            'name': 'Low_Capacity',
            'graph_index': 0,
            'alpha': alpha,
            'capacities': {1: 1, 2: 1, 3: 1, 4: 2, 5: 2, 6: 3, 7: 0},  # Lower capacities
        },
        {
            'name': 'Very_High_Capacity',
            'graph_index': 0,
            'alpha': alpha,
            'capacities': {1: 3, 2: 3, 3: 3, 4: 6, 5: 6, 6: 10, 7: 2},  # Very high capacities
        },

        # MIXED SCENARIOS (capacity variations)
        {
            'name': 'Unbalanced_Capacities',
            'graph_index': 0,
            'alpha': alpha,
            'capacities': {1: 1, 2: 1, 3: 1, 4: 1, 5: 10, 6: 1, 7: 0},  # One very high capacity node
        },
        {
            'name': 'Equal_Capacities',
            'graph_index': 0,
            'alpha': alpha,
            'capacities': {1: 3, 2: 3, 3: 3, 4: 3, 5: 3, 6: 3, 7: 0},  # All equal capacities
        }
    ]
    ...
    for i, config in enumerate(configurations):
        print(f"\n{'-'*60}")
        print(f"RUNNING CONFIGURATION {i+1}/{len(configurations)}: {config['name']}")
        print(f"Alpha: {alpha} | Capacities: {config['capacities']}")
        print(f"{'-'*60}")

        best_solution, all_solutions, success = run_configuration_test(
            graph_index=config['graph_index'],
            alpha=config['alpha'],
            power_capacities_config=config['capacities'],
            node_operational_costs_config={},  # Not used anymore
        ...
        if success and best_solution:
            ...
            print(f"✅ SUCCESS: Score {best_solution.score:.2f}, Connected {len(best_solution.connected_weak)} nodes, Discretionary used: {best_solution.discretionary_used}")
        else:
            results_summary.append({
                'name': config['name'],
                'score': float('inf'),
    ...
    print(f"\n{'='*80}")
    print(f"CONFIGURATION TESTING COMPLETE - ALPHA = {alpha}")
    print(f"{'='*80}")

    print(f"\nDETAILED SUMMARY:")
    print(f"{'Config Name':<25} {'Score':<10} {'Conn':<5} {'Discret Used':<12} {'Total Cap':<10}")
    print(f"{'-'*70}")

    for result in sorted(results_summary, key=lambda x: x['score']):
        status = "✅" if result['failed'] == 0 else "⚠️" if result['failed'] < 999 else "❌"
        print(f"{result['name']:<25} {result['score']:<10.1f} {result['connected']:<5} "
              f"{str(result['discretionary_used']):<12} {result['total_capacity']:<10} {status}")

    # Analysis insights
    successful_results = [r for r in results_summary if r['failed'] == 0]
    if successful_results:
        best_result = min(successful_results, key=lambda x: x['score'])

        print(f"\n🏆 BEST CONFIGURATION:")
        print(f"   {best_result['name']} - Score: {best_result['score']:.2f}")
        print(f"   Used discretionary: {best_result['discretionary_used']}")
        print(f"   Total capacity: {best_result['total_capacity']}")

        # Pattern analysis
        print(f"\n📊 PATTERN ANALYSIS:")
        print(f"   Configurations that used discretionary: {len([r for r in successful_results if r['discretionary_used']])}/{len(successful_results)}")

        discretionary_usage = {}
        for result in successful_results:
            pattern = tuple(sorted(result['discretionary_used']))
            if pattern not in discretionary_usage:
                discretionary_usage[pattern] = []
            discretionary_usage[pattern].append(result['score'])

        print(f"   Most common discretionary patterns:")
        for pattern, scores in sorted(discretionary_usage.items(), key=lambda x: len(x[1]), reverse=True)[:3]:
            avg_score = sum(scores) / len(scores)
            print(f"     {list(pattern)}: {len(scores)} times, avg score {avg_score:.1f}")

    # Save comprehensive logs
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    detailed_log_file = config_logger.save_detailed_log(f"config_analysis_alpha_{alpha}_{timestamp}.txt")
    json_export_file = config_logger.save_json_export(f"config_analysis_alpha_{alpha}_{timestamp}.json")

    print(f"\n📊 COMPREHENSIVE LOGS SAVED:")
    print(f"   📄 Detailed Analysis: {detailed_log_file}")
    print(f"   📄 JSON Export: {json_export_file}")

    if save_individual_files:
        print(f"\n📁 INDIVIDUAL FILES SAVED:")
        print(f"   📊 Each configuration has its own PNG and TXT files")
        print(f"   🎯 Look for files named: GRAPH_0_ConfigName_*.png and GRAPH_0_ConfigName_summary.txt")

    return results_summary

def run_configuration_test(graph_index, alpha, power_capacities_config,
                          node_operational_costs_config, config_name="",
                          save_individual_files=True):
    """
    Run a single configuration test and log results

    Args:
        save_individual_files: If True, saves individual PNG and TXT files (original behavior)
    """
    global config_logger, power_capacities, node_operational_costs
    ...
    config_params = {
        ...
        'alpha': alpha,
        'power_capacities': power_capacities.copy(),
        'timestamp': datetime.datetime.now().isoformat()
    }

    # Start logging this configuration
    config_logger.start_configuration(config_params)
    config_logger.log_message(f"Starting configuration: {config_name}")
    config_logger.log_message(f"Alpha: {alpha}, Graph: {graph_index}")
    ...
    try:
        ...
        with open(file_name, "rb") as f:
            graph = pickle.load(f)
        config_logger.log_message(f"Successfully loaded {file_name}")
        ...
        config_logger.log_message(f"Graph analysis: {len(weak_nodes_list)} weak, {len(mandatory_nodes_list)} mandatory, {len(discretionary_nodes_list)} discretionary")

        # Draw base graph (if saving individual files)
        if save_individual_files:
            draw_graph(graph)
            # Save graph for analysis
            save_graph_for_analysis(graph, graph_index, f"graph_{graph_index}_{config_name}_analysis.json")

        # Remove operational costs - not needed anymore
        # AOC now uses: p_j = power_capacities[j], d_j = node_degree[j]
        ...
        # Log solutions
        for i, solution in enumerate(all_solutions):
            solution_type = solution.graph_info
            config_logger.add_solution(solution, solution_type)

        # Save individual files if requested (original behavior)
        if save_individual_files:
            # Generate unique filename for this configuration
            if config_name:
                base_filename = f"GRAPH_{graph_index}_{config_name.replace(' ', '_')}"
            else:
                base_filename = f"GRAPH_{graph_index}_ALPHA_{alpha}"

            # Visualize and save results (original files)
            visualize_best_solution(graph, best_solution, weak_nodes_list, mandatory_nodes_list,
                                   discretionary_nodes_list, base_filename)

            save_solution_summary(best_solution, all_solutions, f"{base_filename}_summary")

            config_logger.log_message(f"Individual files saved: {base_filename}_*.png, {base_filename}_summary.txt")
        ...
        performance_metrics = {
            ...
            'weak_nodes_count': len(weak_nodes_list),
            'mandatory_nodes_count': len(mandatory_nodes_list),
            'discretionary_nodes_count': len(discretionary_nodes_list),
            'best_score': best_solution.score,
            'best_solution_type': best_solution.graph_info,
            'individual_files_saved': save_individual_files
        }

        config_logger.set_performance_metrics(performance_metrics)
        config_logger.log_message(f"Configuration completed successfully in {execution_time:.2f} seconds")
        config_logger.log_message(f"Best score: {best_solution.score:.2f}")

        return best_solution, all_solutions, True

    except Exception as e:
        config_logger.log_message(f"Configuration failed: {str(e)}", "ERROR")
        return None, [], False

    finally:
        config_logger.finish_configuration()
...
if __name__ == "__main__":
    ...
    if execution_mode == "2":
        ...
        print("🚀 Starting comprehensive configuration testing (logs only)...")
        results = run_multiple_configurations(save_individual_files=False, alpha=alpha)

    elif execution_mode == "3":
        # Run multiple configurations WITH individual files
        print("🚀 Starting comprehensive configuration testing with individual files...")
        print("⚠️  Warning: This will create many PNG and TXT files!")
        confirm = input("Continue? (y/n): ").strip().lower()
        if confirm == 'y':
            results = run_multiple_configurations(save_individual_files=True, alpha=alpha)
        else:
            print("❌ Cancelled")
        ...
        config_params = {
            'config_name': f'Single_Test_Alpha_{alpha}_Graph_{graph_index}',
            'graph_index': graph_index,
            'alpha': alpha,
        ...
        # No need for separate operational costs - using capacities directly as p_j
        # AOC formula: p_j = capacity, d_j = degree, y_j = selected

        print(f"Node capacities (used as p_j in AOC): {power_capacities}")
        ...
        visualize_best_solution(graph, best_solution, weak_nodes_list, mandatory_nodes_list,
                               discretionary_nodes_list, f"GRAPH_{graph_index}_CUSTOM_COST")

        save_solution_summary(best_solution, all_solutions, f"GRAPH_{graph_index}_custom_cost_summary")
        ...
        print(f"  - Custom Cost: {best_solution.acc_cost + best_solution.aoc_cost:.6f}")
        ...
        print(f"\n📁 Output files saved:")
        print(f"  - Graph visualization: GRAPH_{graph_index}_CUSTOM_COST_XXX.png")
        print(f"  - Solution summary: GRAPH_{graph_index}_custom_cost_summary.txt")
